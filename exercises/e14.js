...
export function getOrbitalPeriodsSum(data) {
  ...
  const orbital = ast.filter(asteroids => typeof asteroids.orbitalPeriod === 'number').map(asteroids => asteroids.orbitalPeriod);
  return orbital.reduce((sum, period) => sum + period, 0);
}
